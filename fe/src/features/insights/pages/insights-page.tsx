...
function getBankColor(name: string): string {
   const found = BANK_COLORS.find((b) =>
...
function BankAccountForm({
   ...
   const [color, setColor] = useState(
      account?.color ?? getBankColor(account?.name ?? "") ?? "#4a4a6a",
   );
   ...
   function addAccountNumber(): void {
      ...
      setAccountNumbers((current) =>
         normalizeAccountNumbers([...current, nextAccountNumber]),
      );
   ...
   function removeAccountNumber(numberToRemove: string): void {
      const nextAccountNumbers = accountNumbers.filter(
   ...
   return (
      <form className="bank-form" onSubmit={handleSubmit}>
         ...
         <label className="bank-form__field">
            ...
            <p className="bank-form__hint">
               Nomor yang tampil di sini adalah daftar final yang akan disimpan.
            </p>
         </label>
         <label className="bank-form__field">
            <span>Balance</span>
         ...
         <div className="bank-form__field">
            ...
            <div className="bank-form__color-picker">
               ...
               >
                  <div
                     className="bank-form__color-badge"
                     style={{ backgroundColor: color }}
                  />
                  <span>Select color</span>
                  <IconChevronDown
                     size={14}
                     className="bank-form__color-chevron"
                  />
               </button>
             ...
             <div className="insights-page__desktop-main">
                <div className="bank-accounts-section">
                   ...
                   <div className="bank-cards-wrapper">
                     ...
                     const cardColor =
                        account?.color ?? getBankColor(account?.name ?? "");

                     if (isEditing) {
                        return (
                           <div
                              key={account.id}
                              className="bank-account-form-wrapper"
                           >
                              <BankAccountForm
                                 key={account.id}
                                 account={account}
                     ...
                     return (
                        ...
                        >
                           ...
                           <h2 className="bank-card__amount">
                              {formatAmount(account.calculatedBalance)}
                           </h2>
                           {account.accountNumbers.length > 0 ? (
                              ...
                              <p className="bank-card__numbers bank-card__numbers--empty">
                                 No account numbers
                              </p>
                           )}
